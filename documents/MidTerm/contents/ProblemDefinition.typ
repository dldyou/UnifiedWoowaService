...
따라서 스프링 이벤트(Application Events)를 활용한다. 기본적으로 publisher와 listener가 동작하여 이벤트를 발행하는 메서드를 호출하면 이벤트 리스너가 처리하는 형태이다. 이러한 구조를 통해 서비스간 호출 로직을 제공한다.
...
Kafka를 사용하여 마이크로서비스 간에 발생하는 데이터 동기화 문제를 해결한다.
